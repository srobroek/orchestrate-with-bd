...
`--metadata '{"role":"implementer"}'` (or `reviewer`, `researcher`, `shepherd`, `merger`) and a description
...
| Merger handoff | approved review plus one merge bead with `target`, `base`, `head_sha`, and `receipt` assigned to `pool:orc-merger` | finish and land the run | wave routes to `orc-merger`; its assigned command uses the forge's atomic expected-head guard, including when the head changes after preflight; success or failure terminally closes the attempt and returns cleanup proof before replacement | WORKS (unit) |
...
| 0.5.2 | the delivery gate refused every mutation in the canonical checkout (`merge`, `add`, `apply`, `reset`) unless the repository's own `AGENTS.md` carried the exact authorization line; the lead stopped twice and the human landed the branches | resolved in 0.5.2 by the Worktrunk cutover, which merges nothing in canonical: a lead merges in its own worktree and a feature lands as a PR on GitHub |
