...
| 0.5.0 (V5) | the delivery gate, after its 2026-09-16 hardening, refuses every mutation in the lead's primary checkout (`merge`, `add`, `apply`, `reset`) unless the repository's own `AGENTS.md` carries the exact authorization line; the lead stopped twice and the human landed the branches | open: bead `omp-orchestrate-i3q`; the lead reported it through `report_issue` |
