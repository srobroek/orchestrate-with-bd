...
Keep `.beads.embedded.failed` and `BACKUP_DIR` until the restored source passes parity. The source copy is the only rollback input during this window. Deleting the source copy ends rollback. After deletion, no procedure here can recreate it. The estate has no `astro_plan` server rollback either: the shared server was shut down and `~/.beads/shared-server/` was removed on 2026-09-22 under `omp-orchestrate-osr3.13`. Its data directory held no database subdirectory and no client connection, which is why it was treated as carrying nothing; no SQL enumeration was run against it before shutdown. Retained paths remain available until every linked worktree passes parity.
